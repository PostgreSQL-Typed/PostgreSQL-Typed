...
export class PgTDriver {
	...
	initMappers(): void {
		...
		type ClientWithParser = NodePgClient & { setTypeParser: typeof types.setTypeParser };
		const clients: ClientWithParser[] =
			"_clients" in this.client ? (this.client as { _clients: ClientWithParser[] })._clients : [this.client as ClientWithParser];
